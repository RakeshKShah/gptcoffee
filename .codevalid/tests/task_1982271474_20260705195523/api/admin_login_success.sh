...
USER_ID="user-admin-${CASE_SUFFIX}"
USER_EMAIL="admin-${CASE_SUFFIX}@company.com"
RESPONSE_FILE="/tmp/admin_login_success_${CASE_SUFFIX}.json"
STATUS_FILE="/tmp/admin_login_success_${CASE_SUFFIX}.status"
...
let db = {
  users: [],
  products: [],
  customizations: { sizes: [], milks: [], extras: [] },
  orders: []
};
...
curl -sS -o "$RESPONSE_FILE" -w '%{http_code}' -X POST "$BASE_URL/api/auth/login" \
  ...
  --data "{\"email\":\"${USER_EMAIL}\",\"password\":\"AdminPass456\"}" > "$STATUS_FILE"
...
grep -F '"admin"' "$RESPONSE_FILE" >/dev/null
grep -F "$USER_ID" "$RESPONSE_FILE" >/dev/null

echo "CODEVALID_TEST_ASSERTION_OK:admin_login_success"
