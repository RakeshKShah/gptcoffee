...
USER_ID="user-missing-password-${CASE_SUFFIX}"
USER_EMAIL="customer-${CASE_SUFFIX}@test.com"
RESPONSE_FILE="/tmp/login_missing_password_${CASE_SUFFIX}.json"
STATUS_FILE="/tmp/login_missing_password_${CASE_SUFFIX}.status"
...
db.users.push({ id: userId, name: 'Customer Missing Password', email, password: 'SecurePass123', role: 'customer' });
...
curl -sS -o "$RESPONSE_FILE" -w '%{http_code}' -X POST "$BASE_URL/api/auth/login" \
  ...
  --data "{\"email\":\"${USER_EMAIL}\"}" > "$STATUS_FILE"
...
echo "CODEVALID_TEST_ASSERTION_OK:login_missing_password"
