...
USER_ID="user-missing-email-${CASE_SUFFIX}"
USER_EMAIL="customer-${CASE_SUFFIX}@test.com"
RESPONSE_FILE="/tmp/login_missing_email_${CASE_SUFFIX}.json"
STATUS_FILE="/tmp/login_missing_email_${CASE_SUFFIX}.status"
...
db.users.push({ id: userId, name: 'Customer Missing Email', email, password: 'SecurePass123', role: 'customer' });
...
curl -sS -o "$RESPONSE_FILE" -w '%{http_code}' -X POST "$BASE_URL/api/auth/login" \
  ...
  --data '{"password":"SecurePass123"}' > "$STATUS_FILE"
...
echo "CODEVALID_TEST_ASSERTION_OK:login_missing_email"
