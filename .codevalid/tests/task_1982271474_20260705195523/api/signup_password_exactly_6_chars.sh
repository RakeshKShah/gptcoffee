...
EMAIL="six.chars.${CASE_SUFFIX}@example.com"
TMP_DIR="$(mktemp -d)"
...
{"name":"Six Chars","email":"${EMAIL}","password":"123456"}
EOF
...
grep -F '"name":"Six Chars"' "$RESPONSE_FILE" >/dev/null
grep -F '"email":"'"$EMAIL"'"' "$RESPONSE_FILE" >/dev/null
grep -F '"role":"buyer"' "$RESPONSE_FILE" >/dev/null

echo "CODEVALID_TEST_ASSERTION_OK:signup_password_exactly_6_chars"
