...
CASE_SUFFIX="$(date +%s)-$$"
USER_NAME="Missing Email ${CASE_SUFFIX}"
...
# Given — create a valid account, then omit email in the login request
...
# When — attempt login without the email field
STATUS="$(curl -sS -o "$RESPONSE_FILE" -w '%{http_code}' -X POST "$BASE_URL/api/auth/login" \
  -H 'Content-Type: application/json' \
  --data "$(printf '{"password":"%s"}' "$USER_PASSWORD")")"
...
printf 'CODEVALID_TEST_ASSERTION_OK:missing_email_field\n'
